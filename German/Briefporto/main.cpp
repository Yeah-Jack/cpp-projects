...
int main() {
	...
	if (gewichtUser <= gewichtStandardbrief && laengeUser <= laengeStandardbrief && breiteUser <= breiteStandardbrief && hoeheUser <= hoeheStandardbrief) {
		std::cout << "Dein Brief ist ein Standardbrief." << std::endl;
	} else if (gewichtUser <= gewichtKompaktbrief && laengeUser <= laengeKompaktbrief && breiteUser <= breiteKompaktbrief && hoeheUser <= hoeheKompaktbrief) {
		std::cout << "Dein Brief ist ein Kompaktbrief." << std::endl;
	} else if (gewichtUser <= gewichtGrossbrief && laengeUser <= laengeGrossbrief && breiteUser <= breiteGrossbrief && hoeheUser <= hoeheGrossbrief) {
		std::cout << "Dein Brief ist ein Großbrief." << std::endl;
	} else if (gewichtUser <= gewichtMaxibrief && laengeUser <= laengeMaxibrief && breiteUser <= breiteMaxibrief && hoeheUser <= hoeheMaxibrief) {
