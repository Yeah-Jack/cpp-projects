...
void updateFamilyMember(Family &family, int index, int newAge, double newHeight, double newWeight);
...
void updateFamilyMember(Family &family, int index, int newAge, double newHeight, double newWeight) {
    if (index >= 0 && index < family.familySize) {
        ...
        std::cout << "Updated information for " << family.members[index].name << std::endl;
    } else {
